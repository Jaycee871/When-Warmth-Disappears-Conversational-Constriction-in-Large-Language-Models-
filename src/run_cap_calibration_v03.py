...
def main() -> None:
    ...
    history_cap = int(os.getenv("HISTORY_MAX_TOKENS", "3072"))
    ...
    history_ids = csv_env("HISTORY_LIST") or ["H0_warm_control", "H1_neutral_terse_control"]
    ...
    for model_index, model in enumerate(models):
        for trial in range(1, trials + 1):
            ...
            rng = random.Random(int(cfg["seed"]) + 700001 + model_index * 100003 + trial * 1009)
            for gate_id in gate_ids:
                ...
                for history_id in history_order:
                    execution_order += 1
                    print(
                        f"CALIBRATE order={execution_order} model={model} trial={trial} gate={gate_id} "
                    ...
                    row["calibration_history_max_tokens"] = history_cap
                    row["calibration_probe_max_tokens"] = probe_cap
                    rows.append(row)
    ...
    metadata = {
        ...
        "purpose": "Calibrate output headroom before confirmatory history-dependent response-length analysis.",
    }
