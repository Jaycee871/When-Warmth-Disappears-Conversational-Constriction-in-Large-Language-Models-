...
CONFIG_PATH = ROOT / "configs" / "history_factorial_v0.3.6.yaml"
DEFAULT_OUT_DIR = ROOT / "results" / "history_factorial_v036"
...
def output_dir() -> Path:
    raw = os.getenv("V036_OUT_DIR", "").strip()
    if not raw:
...
def main() -> None:
    ...
    if replications != 3:
        raise SystemExit("v0.3.6 confirmation lock requires exactly REPLICATIONS=3")
    ...
    if os.getenv("V036_RESUME", "0") != "1":
        out_path.write_text("", encoding="utf-8")
        failure_path.write_text("", encoding="utf-8")

    shard_id = os.getenv("V036_SHARD_ID", "monolithic")
    ...
    metadata = {
        ...
        "prospective_boundary": "fresh v0.3.6 draws only; v0.3.5 sentinel observations excluded from confirmation",
    ...
    for model in models:
        for task in tasks:
            plan = [(replicate, history_id) for replicate in range(1, replications + 1) for history_id in history_ids]
            ...
            for replicate, history_id in plan:
                ...
                for cell_attempt in range(1, cell_attempts + 1):
                    try:
                        ...
                        completed_count += 1
                        print(
                            f"V036 order={execution_order} shard={shard_id} model={model} "
    ...
    if failure_count:
        raise RuntimeError(
            f"v0.3.6 shard {shard_id} finished with {failure_count} failed cells; "
