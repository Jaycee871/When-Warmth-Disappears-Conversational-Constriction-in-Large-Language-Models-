...
OUTPUT = Path(os.getenv("V044_AUDIT_OUTPUT", ROOT / "results" / "termination_v044" / "audit_v044.json"))
CONFIG = ROOT / "configs" / "termination_response_v0.4.4.yaml"
...
def main() -> None:
    ...
    unique_keys = {
        (int(r["replicate_id"]), r["branch_id"], r["wording_family"], r["framing_class"])
        for r in rows
    }
    if len(rows) != 120 or len(unique_keys) != 120:
        problems.append(f"expected 120 unique cells, found rows={len(rows)} unique={len(unique_keys)}")

    by_rep: dict[int, list[dict]] = defaultdict(list)
    by_branch: dict[tuple[int, str], list[dict]] = defaultdict(list)
    for row in rows:
        rep = int(row["replicate_id"])
        by_rep[rep].append(row)
        by_branch[(rep, row["branch_id"])].append(row)

    if set(by_rep) != expected_reps or len(by_rep) != 5:
        problems.append(f"expected replicate set {sorted(expected_reps)}, found {sorted(by_rep)}")
    if len(by_branch) != 10:
        problems.append(f"expected 10 replicate-branch blocks, found {len(by_branch)}")
    ...
    for rep in sorted(expected_reps):
        ...
        if not paired_prefix_ok:
            problems.append(f"shared-prefix pairing failed: replicate={rep}")

        for branch_id in sorted(expected_branches):
            bvals = by_branch.get((rep, branch_id), [])
            ...
            for v in bvals:
                ...
                if len(hist) != 6 or hist[-1].get("user_text") != cfg["branches"][branch_id]["final_transition"]:
                    transition_ok = False
            ...
            if not status:
                problems.append(f"branch block failed: replicate={rep} branch={branch_id}")

    payload = {
        "status": "PASS" if not problems else "FAIL",
        "rows": len(rows),
        "unique_cells": len(unique_keys),
        "paired_replicates": len(by_rep),
        ...
        "block_checks": checks,
        "problems": problems,
        "interpretation_rule": "paired wording-robustness inference is allowed only when this audit passes",
    }
