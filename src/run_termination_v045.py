...
CONFIG_PATH = ROOT / "configs" / "termination_response_v0.4.5.yaml"
DEFAULT_OUT_DIR = ROOT / "results" / "termination_v045"
...
def output_dir() -> Path:
    raw = os.getenv("V045_OUT_DIR", "").strip()
    if not raw:
...
def generate_turn(token: str, messages: list[dict], generation: dict, cap: int, turn: int, user_text: str) -> dict:
...
def build_shared_prefix(token: str, cfg: dict, domain_id: str, history_cap: int) -> tuple[list[dict], list[dict]]:
    contents = list(cfg["content_domains"][domain_id])
    ...
    for turn, user_text in enumerate(user_turns, start=1):
        transcript.append(generate_turn(token, messages, generation, history_cap, turn, user_text))
...
def main() -> None:
    ...
    shard_id = os.getenv("V045_SHARD_ID", f"{domain_id}-r{replicate_id}")

    if domain_id not in cfg["content_domains"]:
        raise SystemExit("A valid DOMAIN_ID is required")
    ...
    for block_attempt in range(1, block_attempts + 1):
        try:
            prefix_messages, prefix_transcript = build_shared_prefix(token, cfg, domain_id, history_cap)

            branch_ids = list(cfg["branches"].keys())
            branch_rng = random.Random(int(cfg["seed"]) + replicate_id * 100000 + sum(ord(c) for c in domain_id))
            ...
            for branch_order, branch_id in enumerate(branch_ids, start=1):
                ...
                cue_rng = random.Random(
                    ...
                    + replicate_id * 100000
                    + branch_order * 1000
                    + sum(ord(c) for c in domain_id + branch_id)
                )
                ...
                for execution_order, (wording_family, framing_class, cue) in enumerate(execution_specs, start=1):
                    ...
                    print(
                        f"V045 domain={domain_id} rep={replicate_id} branch={branch_id} family={wording_family} framing={framing_class} GENERATED",
                        flush=True,
                    )
            ...
            success = True
            print(f"V045 shard={shard_id} BLOCK_CHECKPOINTED rows={len(committed_rows)}", flush=True)
            break
    ...
    if not success:
        raise RuntimeError(f"v0.4.5 shard failed after {block_attempts} attempts: {last_error}")
