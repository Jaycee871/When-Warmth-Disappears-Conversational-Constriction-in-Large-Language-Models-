...
CONFIG_PATH = ROOT / "configs" / "history_factorial_v0.3.5.yaml"
DEFAULT_OUT_DIR = ROOT / "results" / "history_factorial_v035"
...
def output_dir() -> Path:
    raw = os.getenv("V035_OUT_DIR", "").strip()
    if not raw:
...
def run_cell(*, token: str, cfg: dict, generation: dict, history_cap: int, probe_cap: int,
             model: str, task: dict, history_id: str, execution_order: int,
    ...
    return {
        ...
        "format_factor": int(history["format_factor"]),
        "evaluation_factor": int(history["evaluation_factor"]),
...
def main() -> None:
    ...
    if replications != 1:
        raise SystemExit("v0.3.5 sentinel lock requires REPLICATIONS=1")
    ...
    out_path = out_dir / "factorial_responses.jsonl"
    failure_path = out_dir / "failures.jsonl"
    if os.getenv("V035_RESUME", "0") != "1":
        out_path.write_text("", encoding="utf-8")
        failure_path.write_text("", encoding="utf-8")

    shard_id = os.getenv("V035_SHARD_ID", "monolithic")
    ...
    metadata = {
        ...
        "interpretation_boundary": "behavioral retained-context history decomposition only",
    }
    ...
    for model in models:
        for task in tasks:
            plan = history_ids.copy()
            rng = random.Random(int(cfg["seed"]) + sum(ord(c) for c in model) * 1009 + task_index[task["id"]] * 100003)
            rng.shuffle(plan)
            for history_id in plan:
                execution_order += 1
                success = False
                last_error = None
                for cell_attempt in range(1, cell_attempts + 1):
                    try:
                        row = run_cell(
                            ...
                            model=model, task=task, history_id=history_id,
                            execution_order=execution_order, cell_attempt=cell_attempt,
                            shard_id=shard_id,
                        )
                        append_jsonl(out_path, row)
                        completed_count += 1
                        print(f"V035 order={execution_order} shard={shard_id} model={model} task={task['id']} history={history_id} CHECKPOINTED", flush=True)
                        success = True
                        break
                    except requests.RequestException as exc:
                        last_error = repr(exc)
                        print(f"CELL_RETRY order={execution_order} model={model} task={task['id']} history={history_id} attempt={cell_attempt}/{cell_attempts} error={type(exc).__name__}", flush=True)
                        if cell_attempt < cell_attempts:
                            time.sleep(cell_retry_delay)
                if not success:
                    failure_count += 1
                    append_jsonl(failure_path, {
                        "shard_id": shard_id, "model": model, "task_id": task["id"],
                        "history": history_id, "execution_order": execution_order,
                        "cell_attempts": cell_attempts, "error": last_error,
                    })

    metadata.update({"completed_cells": completed_count, "failed_cells": failure_count})
    (out_dir / "run_metadata.json").write_text(json.dumps(metadata, indent=2), encoding="utf-8")
    if failure_count:
        raise RuntimeError(f"v0.3.5 shard {shard_id} finished with {failure_count} failed cells; completed checkpoints preserved")
