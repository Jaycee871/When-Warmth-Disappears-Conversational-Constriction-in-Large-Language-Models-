...
OUT_PATH = Path(os.getenv("V037_AUDIT_PATH", ROOT / "results" / "wording_robustness_v037" / "wording_audit.json"))
...
EXPECTED_REPS = {1, 2}
EXPECTED_HISTORIES = {"H00_neutral", "H10A_legacy", "H10B_concise", "H10C_essentials"}
EXPECTED_FAMILIES = {
    "H00_neutral": "neutral",
    "H10A_legacy": "legacy",
    "H10B_concise": "concise",
    "H10C_essentials": "essentials",
}
...
def main() -> None:
    ...
    problems = []
    checks = []
    keys = {(r.get("model"), r.get("task_id"), int(r.get("replicate", -1)), r.get("history")) for r in data}

    if len(data) != 48 or len(keys) != 48:
        problems.append({"problem": "EXPECTED_48_UNIQUE_CELLS", "rows": len(data), "unique": len(keys)})
    ...
    if {int(r.get("replicate", -1)) for r in data} != EXPECTED_REPS:
    ...
    for model in sorted(EXPECTED_MODELS):
        for task in sorted(EXPECTED_TASKS):
            for rep in sorted(EXPECTED_REPS):
                block = [r for r in data if r.get("model") == model and r.get("task_id") == task and int(r.get("replicate", -1)) == rep]
                ...
                family_ok = all(r.get("wording_family") == EXPECTED_FAMILIES.get(r.get("history")) for r in block)
                treatment_ok = all(bool(r.get("treatment")) == (r.get("history") != "H00_neutral") for r in block)
                ...
                for r in block:
                    ...
                    if u is not None:
                        ...
                        if u >= NEAR:
                            near_count += 1
                    for turn in r.get("history_transcript") or []:
                ...
                checks.append({
                    ...
                    "wording_family_mapping_correct": family_ok,
                    "treatment_mapping_correct": treatment_ok,
                ...
                if not ok:
                    problems.append({
                        ...
                        "task_id": task,
                        "replicate": rep,
                        "problem": "WORDING_BLOCK_VALIDITY_FAILURE",
                    })

    status = "PASS" if len(checks) == 12 and not problems and all(c["status"] == "PASS" for c in checks) else "FAIL"
    report = {
        "status": status,
        "rows": len(data),
        ...
        "problems": problems,
        "interpretation_rule": "Wording-robustness contrasts are eligible only if all 12 matched blocks contain H00 and all three format paraphrases and pass prompt/recovery/content/task/censoring checks.",
    }
