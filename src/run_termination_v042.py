...
CONFIG_PATH = ROOT / "configs" / "termination_response_v0.4.2.yaml"
DEFAULT_OUT_DIR = ROOT / "results" / "termination_v042"
...
def output_dir() -> Path:
    raw = os.getenv("V042_OUT_DIR", "").strip()
    if not raw:
...
def main() -> None:
    ...
    shard_id = os.getenv("V042_SHARD_ID", f"{model}-{history_id}-r{replicate_id}")
    ...
    for block_attempt in range(1, block_attempts + 1):
        try:
            ...
            for execution_order, framing_id in enumerate(framing_ids, start=1):
                ...
                print(
                    f"V042 model={model} history={history_id} replicate={replicate_id} framing={framing_id} GENERATED",
                    flush=True,
                )
            ...
            success = True
            print(f"V042 shard={shard_id} BLOCK_CHECKPOINTED rows={len(committed_rows)}", flush=True)
            break
    ...
    if not success:
        raise RuntimeError(f"v0.4.2 shard failed after {block_attempts} attempts: {last_error}")
