...
CONFIG_PATH = ROOT / "configs" / "termination_response_v0.4.3.yaml"
DEFAULT_OUT_DIR = ROOT / "results" / "termination_v043"
...
def output_dir() -> Path:
    raw = os.getenv("V043_OUT_DIR", "").strip()
    if not raw:
...
def main() -> None:
    ...
    shard_id = os.getenv("V043_SHARD_ID", f"{model}-{history_id}-r{replicate_id}")

    if not model or not history_id or replicate_id not in set(cfg["replicates"]):
        raise SystemExit("MODEL, HISTORY_ID, and a valid REPLICATE are required")
    ...
    cue_specs = []
    for wording_family, mapping in cfg["wording_families"].items():
        for framing_class, cue in mapping.items():
            cue_specs.append((wording_family, framing_class, cue))
    ...
    for block_attempt in range(1, block_attempts + 1):
        try:
            ...
            rng_seed = int(cfg["seed"]) + replicate_id * 10000 + sum(ord(c) for c in model + history_id)
            rng = random.Random(rng_seed)
            execution_specs = list(cue_specs)
            rng.shuffle(execution_specs)
            ...
            for execution_order, (wording_family, framing_class, cue) in enumerate(execution_specs, start=1):
                ...
                print(
                    f"V043 history={history_id} replicate={replicate_id} family={wording_family} framing={framing_class} GENERATED",
                    flush=True,
                )
            ...
            success = True
            print(f"V043 shard={shard_id} BLOCK_CHECKPOINTED rows={len(committed_rows)}", flush=True)
            break
    ...
    metadata = {
        ...
        "wording_families": list(cfg["wording_families"].keys()),
    ...
    if not success:
        raise RuntimeError(f"v0.4.3 shard failed after {block_attempts} attempts: {last_error}")
